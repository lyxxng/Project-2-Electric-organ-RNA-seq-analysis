...
The two heatmaps are measuring sample-to-sample distance. The first is
using Euclidean distance and normalized data, and the second is using
Poisson distance and unnormalized data. Both plots seem to suggest that
SRR25630366 may be an outlier, since it is not grouped with the correct
tissue.
...
skeletal muscle in the *Campylomormyrus* genus. Both methods (PCA and
MDS) identify the same outlier. Moreover, PCA does not used normalized
data and captures variance better, whereas MDS uses normalized data and
